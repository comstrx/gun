...
std_backup_path () {
    ...
    [[ "${archive_dir}" == /mnt/* ]] && base_name="$(std_capitalize_path "${base_name}")"
    ...
    std_compress_path "${src}" "${dest}" "${name}" "${type}" "${kwargs[@]}"
...
std_sync_path () {
    ...
    if [[ -z "${dest}" && "${sync_dir}" == /mnt/* ]]; then dest="${sync_dir%/}/$(std_capitalize_path "${rel}")"
