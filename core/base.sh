...
[[ "${BASH_SOURCE[0]}" != "${0}" ]] || { printf "%s\n" "base.sh: this file should not be run externally." >&2; exit 2; }
[[ -n "${BASH_VERSION:-}" ]] || { printf '%s\n' "base.sh: Bash required." >&2; return 2 2>/dev/null || exit 2; }
[[ -n "${BASE_LOADED:-}" ]] && return 0

BASE_LOADED=1 YES="${YES:-0}" QUIET="${QUIET:-0}" VERBOSE="${VERBOSE:-0}"
